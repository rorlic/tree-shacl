...
describe('Test TR5.7 (tree:value usage)', async () => {
  const validator = await createValidator(['tree-common.ttl', 'TR5.7-remaining-items-usage.ttl']);
  ...
  it('may contain a remainingItems predicate', async () => {
    expectInfo({ sourceShape: suggestedShape} as ExpectedResult,
      await validateFile('./tests/TR5.7/no-remaining-items.ttl', validator));
  });

  it('contains a remainingItems predicate', async () => {
    expectNoInfo(suggestedShape,
      await validateFile('./tests/TR5.7/with-remaining-items.ttl', validator));
  });

  it('must have remainingItems as integer value', async () => {
    expectViolation({ sourceShape: invalidShape } as ExpectedResult,
      await validateFile('./tests/TR5.7/iri-remaining-items.ttl', validator));
  });

  it('must have a single remainingItems predicate', async () => {
    expectViolation({ sourceShape: invalidShape } as ExpectedResult,
      await validateFile('./tests/TR5.7/multiple-remaining-items.ttl', validator));
  });
});
